...
export async function nativeTool(name: string, args: unknown): Promise<ServerResult> {
    ...
    if(['native_click','native_type','native_key'].includes(name) && await configManager.getValue('nativeControlEnabled')!==true)
    ...
    try {
        const result=await new Promise<any>((resolve,reject)=>{
            const child=execFile(executable,[],{timeout:20000,maxBuffer:24*1024*1024},(error,stdout)=>{
            ...
            child.stdin?.end(JSON.stringify({operation:name.replace('native_',''),arguments:parsed.data}));
        });
        if(result.image){const {image,...metadata}=result;return {content:[{type:'text',text:JSON.stringify(metadata)},{type:'image',data:image,mimeType:result.mimeType}]};}
